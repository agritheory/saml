...
def get_context(context):
	...
	for provider in saml_logins:
		...
		if not redirect_to:
			auth_url = frappe.utils.get_url(f"/api/method/saml.saml.login?provider={provider.name}")
		else:
			auth_url = frappe.utils.get_url(
				f"/api/method/saml.saml.login?provider={provider.name}&redirect-to={redirect_to}"
			)
